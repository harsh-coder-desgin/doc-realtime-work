  ...
  const createnewdoc = async (ID) => {
    ...
    if (orgname) {
      const createdoc = await authdoc.orgcreatedoc({
        ...
        createuserid: orgname.data.data.createuserid, createrdocusername: orgname.data.data.createrdocusername,
        id: id
      })
      if (createdoc) {
        localStorage.setItem("OrgDoc", createdoc.data.data._id)
      }
    }
  }
